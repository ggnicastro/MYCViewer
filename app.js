(() => {
  ...
  async function loadPair(pdbSource, yamlSource, sourceKind) {
    ...
    setBusy(true);
    showOverlay('loading', 'Loading PDB + YAML', 'Reading files, validating residue selections, and building the component-only Mol* scene…');
    ...
    try {
      ...
      showToast(
        ...
        (warnings ? ` with ${warnings} selection warning${warnings === 1 ? '' : 's'}.` : '.')
      );
      return true;
    } catch (error) {
      if (generation !== state.loadGeneration) return false;
  ...
  function renderCurrentState() {
    ...
    elements.sourceSummary.textContent =
      `${current.pdbName} + ${current.yamlName} · ${current.pdbInfo.atomCount.toLocaleString()} polymer atoms · ` +
...
`;
  ...
  async function initializeViewer() {
    ...
    elements.brandSubtitle.textContent = 'PDB + YAML component representations';
